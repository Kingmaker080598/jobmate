  ...
  const downloadFile = async (url, filename, targetFormat = 'original', resumeId = null) => {
    ...
    if (targetFormat === 'pdf' && ext === 'docx') {
      try {
        ...
        alert('❌ Failed to convert and download PDF.');
        console.error(err);
      } finally {
  ...
  return (
    <RequireAuth>
      ...
      <div className="min-h-screen bg-gradient-to-br from-black via-slate-900 to-indigo-950 text-white px-6 py-20">
        ...
        >
          <h1 className="text-4xl font-bold text-white mb-2">📂 Your Resume History</h1>
          <p className="text-gray-400">View and download all your previously generated or uploaded resumes.</p>
        </motion.div>
            ...
            {resumes.map((r) => (
              ...
              >
                <div className="flex justify-between items-center mb-2">
                  <h2 className="text-lg font-bold text-white">{r.job_title}</h2>
                ...
                <div className="flex flex-wrap gap-3">
                  ...
                  <button
                    onClick={() => downloadFile(r.resume_url, r.job_title.replace(/\s+/g, '_'), 'pdf', r.id)}
